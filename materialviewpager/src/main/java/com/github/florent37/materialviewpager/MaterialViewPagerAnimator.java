...
public class MaterialViewPagerAnimator {
    ...
    MaterialViewPagerAnimator(MaterialViewPager materialViewPager) {
        ...
        setScale(((float)1.1) * ((float)1.1 - mHeader.finalScale) + mHeader.finalScale, mHeader.mLogo);
        ...
        percent = minMax(0, percent, 1);
        {
            ...
            if (mHeader.mLogo != null) { //move the header logo to toolbar
                ...
                if (this.settings.hideLogoWithFade) {
                    ...
                    float scale;
                    if(percent!=1){
                        scale = ((float)1.1 - percent) * ((float)1.1 - mHeader.finalScale) + mHeader.finalScale;
                        setScale(scale, mHeader.mLogo);
                    }
                    else
                        scale = ((float)1.38 - percent) * ((float)1.38 - mHeader.finalScale) + mHeader.finalScale;
                        setScale(scale, mHeader.mLogo);
                    }
            }
